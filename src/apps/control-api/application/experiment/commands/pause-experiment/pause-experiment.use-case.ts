...
import { PauseExperimentCommand } from './pause-experiment.command';

@Injectable()
export class PauseExperimentUseCase {
  constructor(
  ...
  async execute(
    command: PauseExperimentCommand,
  ): Promise<
    Result<
      void,
      | NotFoundError
  ...
  > {
    return this.transactionManager.execute(async () => {
      ...
      const actionResult = experiment.pause();
